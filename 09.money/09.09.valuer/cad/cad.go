...
func (receiver *CAD) UnmarshalJSON(b []byte) error {
	parsed, _ := ParseCAD(string(b))
	receiver.cents = parsed.AsCents()
	return nil
}

func (receiver CAD) Value() (driver.Value, error) {
	return driver.Value(receiver.AsCents()), nil
}

func (receiver *CAD) Scan(src interface{}) error {
	value, ok := src.(int64)
	if !ok {
		return errors.New("can convert source to int64 for CAD struct")
	}
	receiver.cents = value
	return nil
}

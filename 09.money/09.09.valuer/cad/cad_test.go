...
func TestMarshalJSON(t *testing.T) {
	...
	myJson := struct {
		Wallet CAD `json:"wallet"`
	}{
		Wallet: wallet,
	}

	printed, _ := json.Marshal(myJson)

	if string(printed) != `{"wallet":"CAD$2.40"}` {
		t.Errorf(`expected '{"wallet":"CAD$2.40"}' but received %s`, printed)
	}
}
...
func TestUnmarshalJSON(t *testing.T) {
	...
	if err != nil || w.Wallet.AsCents() != 240 {
		t.Errorf(`error in unmarshal execution`)
	}
}
...
func TestValue(t *testing.T) {
	...
	if val != int64(240) {
		t.Errorf("expected 240 but received %d", val)
	}
}
